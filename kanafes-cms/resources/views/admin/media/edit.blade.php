...
@section('title', 'Sửa Video / Mạng xã hội')
        ...
        <form action="{{ route('admin.media.update', $medium) }}" method="POST">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <div class="flex gap-4">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="type" value="youtube" {{ $medium->type === 'youtube' ? 'checked' : '' }} required class="text-red-600 focus:ring-red-500">
                            <span class="text-sm">🎬 YouTube</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="type" value="facebook" {{ $medium->type === 'facebook' ? 'checked' : '' }} required class="text-blue-600 focus:ring-blue-500">
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $medium->title) }}" required
                ...
                <div>
                    ...
                    <textarea name="url" required rows="3"
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-red-400 focus:border-transparent">{{ old('url', $medium->url) }}</textarea>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Thứ tự hiển thị</label>
                    <input type="number" name="sort_order" value="{{ old('sort_order', $medium->sort_order) }}"
                           class="w-32 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-red-400">
                </div>

                <div class="flex items-center gap-3 mt-2">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ $medium->is_active ? 'checked' : '' }}
