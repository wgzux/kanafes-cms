...
@section('title', 'Sửa công ty đối tác')
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">
                        ...
                        <span class="text-xs text-gray-400 font-normal">(Enter để xuống dòng)</span>
                    </label>
                    <textarea name="name" required rows="3"
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-red-400 focus:border-transparent">{{ old('name', $partner->name) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Logo hiện tại</label>
                    @if($partner->logo)
                        <img src="{{ $partner->logo_url }}" alt="{{ $partner->name }}"
                             class="h-20 object-contain rounded border mb-3 bg-gray-50 p-1">
                    @endif
